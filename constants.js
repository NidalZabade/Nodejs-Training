export let CURRENCIES_URL =
  "https://gist.githubusercontent.com/ksafranski/2973986/raw/5fda5e87189b066e11c1bf80bbfbecb556cf2cc1/Common-Currency.json";
export let COUNTRIES_URL =
  "https://gist.githubusercontent.com/tiagodealmeida/0b97ccf117252d742dddf098bc6cc58a/raw/f621703926fc13be4f618fb4a058d0454177cceb/countries.json";
export let COUNTRY_NOT_FOUND = "Country not found";
export let CURRENCY_NOT_FOUND = "Currency not found";
export let LINE = "----------------------------------------";
export let WEEK_DAYS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
export const DAY_IN_MILLISECOND = 1000 * 60 * 60 * 24;
export const MINUTES_IN_ONE_HOUR = 60;
export const MONTHS_IN_QUARTER = 3;
export const INCREMENT_ONE_DAY = 1;
export const DECREMENT_ONE_DAY = -1;

